...
cat(sprintf("Zeilen: %d -> Train=%d, Tune=%d, Eval=%d, Labels=%d, Features=%d\n",
            n, length(train_ids), length(tune_ids), length(eval_ids), length(label_cols), length(feature_cols)))
...
pool <- binary_relevance_pool(train, feature_cols, label_cols, train_ids,
                               list(tune = tune_ids, eval = eval_ids), learner_constructor, seed = seed)

truth_mat <- as.matrix(train[eval_ids, label_cols, with = FALSE])
pred_mat_default <- matrix(NA_integer_, nrow = length(eval_ids), ncol = length(label_cols), dimnames = list(NULL, label_cols))
pred_mat_tuned <- matrix(NA_integer_, nrow = length(eval_ids), ncol = length(label_cols), dimnames = list(NULL, label_cols))
...
for (j in seq_along(label_cols)) {
  ...
  truth_tune01 <- as.integer(train[[lbl]][tune_ids])
  truth_eval01 <- as.integer(train[[lbl]][eval_ids])

  best_thr <- tune_threshold_accuracy(prob_tune, truth_tune01, multilabel_threshold_grid)
  pred_mat_default[, j] <- as.integer(prob_eval >= 0.5)
  pred_mat_tuned[, j] <- as.integer(prob_eval >= best_thr)
  ...
  n_positive_train <- sum(train[[lbl]][train_ids] == 1)
  per_label_results[[j]] <- data.table(
    label = lbl, base_rate = mean(truth_eval01), n_positive_train = n_positive_train,
    best_threshold = best_thr, accuracy_default = acc_default, accuracy_tuned = acc_tuned
  )
  cat(sprintf("  %s: Basisrate=%.3f  n_pos_train=%d  Schwelle=%.2f  Accuracy 0.5->%.3f getunt->%.3f\n",
              lbl, mean(truth_eval01), n_positive_train, best_thr, acc_default, acc_tuned))
...
cat("\n=== Multi-Label-Metriken: Default (0.5) vs. Accuracy-getunte Schwelle ===\n")
