...
| `review_literature_split_candidates.R` | Manuelle Pruefliste fuer `split_match_candidate`-Zeilen; dokumentiert fuer `credit-g`, warum trotz F1/10-fold-Match kein Upgrade aus `context_only` erfolgt |
...
| `115_adversarial_validation.R` | Prueft per Adversarial Validation, ob Train und Test unterscheidbar sind (AUC + Feature Importance); zusaetzlich univariate Drift-Tests je Feature (`univariate_drift.R`: KS/Chi2, BH-korrigiert, mit Effektgroesse), ESS/n der OOF-Propensity-Gewichte (Reweighting-Machbarkeit) und optionale Stufen (`adversarial_staged_exclude`), die verdaechtige Feature-Gruppen ausschliessen, um die Shift-treibende Gruppe zu isolieren |
...
| `148_ensemble_candidate_pool.R` | Laedt das Modelle-Artefakt: trainiert einen Pool aus 24 Kandidaten (Ranger/LightGBM/CatBoost, variierte Hyperparameter), speichert Wahrscheinlichkeits-Matrizen aller Kandidaten auf dem Eval-Split fuer `149_ensemble_selection.R` |
| `149_ensemble_selection.R` | Caruana-Greedy-Ensemble-Selection (Caruana et al. 2004) auf dem Kandidaten-Pool: gieriger Vorwaertsauswahl mit Wiederholung, klassenstratifizierter Selektions-/Bestaetigungs-Split, vergleicht bestes Einzelmodell vs. gleichgewichteten Blend vs. Greedy-Ensemble |
...
1. **Feature-Importance-Konzentration**: ein LightGBM-Fit, traegt ein einzelnes Feature `> leak_audit_importance_share_threshold` (Default 50%) der Gain-Importance? Das staerkste Feature wird IMMER mit exakter Zahl + qualitativer Einordnung ausgegeben (`<10%` KLEIN, `10-30%` MITTEL, `30-50%` HOCH - nur Hinweis, `>=leak_audit_importance_share_threshold` SEHR HOCH - WARNUNG). Die HOCH-Stufe (`leak_audit_advisory_share_threshold`, Default 30%) loest NICHTS in Schritt 3/4 aus, ist rein informativ - bewusst so kalibriert, nachdem eine Suche ueber 17 reale Projekte zeigte, dass 7 davon zwischen 30-50% liegen und alle legitim sind (eine niedrigere HARTE Schwelle haette die "selten, aber berechtigt"-Eigenschaft des Guards verwaessert, siehe TARGETS.md). Eine **kumulative Top-k-Erweiterung** (`leak_audit_cumulative_max_k`/`leak_audit_cumulative_share_threshold`, Default 98%) erweitert einen BEREITS bestehenden Einzelverdacht um knapp-darunter-liegende Partner (Leak-PAAR/-GRUPPE, z.B. `casual`+`registered` bei Bike-Sharing) - sie erzeugt NIE einen neuen Verdacht aus einer sauberen Verteilung.
   **Schritt 1b - Korrelierte Feature-Cluster** (2026-08-21, aus `lending-club-leak-test`): faengt einen Leak, der ueber viele MITEINANDER REDUNDANTE Features verteilt ist, bei dem weder die Einzelschwelle noch die kumulative Erweiterung greifen (Gain-Share ist bei Redundanz irrefuehrend - LightGBM summiert Gain ueber alle Splits, ein Feature mit wenigen, aber sehr entscheidenden fruehen Splits sammelt weniger Gain an als Features, die fuer viele feine Splits wiederverwendet werden). Numerische Features werden nach Korrelation geclustert (`leak_audit_cluster_correlation_threshold`, Default 0.5); nur der Cluster mit der groessten summierten Gain-Importance wird per Retraining geprueft (hoechstens 1 zusaetzliches Retraining, nur wenn die Summe schon die Advisory-Schwelle ueberschreitet), und nur ein Score-Abfall `> leak_audit_cluster_drop_threshold` (Default 15 Prozentpunkte) zaehlt als echter Verdacht. **Bekannte Grenze, nicht vollstaendig geloest** (siehe TARGETS.md): am Anlassfall selbst (10 stark redundante Post-Outcome-Felder, BAcc 0.9983->0.5317 ehrlich) zeigt der Check zwar ein informatives 7.9-Punkte-Signal, ueberschreitet die Warnschwelle aber NICHT - Korrelationscluster mit einer festen Schwelle vermischen bei niedrigem Threshold legitime mit leckenden Features, bei hohem Threshold fragmentieren sie zu klein. Eine getestete Alternative (iterative Einzelfeature-Entfernung ueber mehrere Runden) scheiterte am selben Extremfall noch deutlicher (nach 11 Runden/6 von 10 Leak-Feldern entfernt blieb BAcc bei 0.97) und waere zudem viel teurer. Bewusst trotzdem behalten: eine echte, wenn auch unvollstaendige Verbesserung fuer weniger extreme Faelle, bei sauberen Projekten praktisch kostenlos (nur eine Korrelationsmatrix, kein zusaetzliches Retraining ausser bei einem bereits ueber-der-Advisory-Schwelle liegenden Cluster).
...
4. **Ehrlich-vs-aufgeblasen-Zerlegung**: gepaarter Holdout-Split, Zielmetrik (`baseline_measure_ids`) mit vs. ohne die Verdaechtigen aus Schritt 1+1b+2.
