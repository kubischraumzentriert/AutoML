...
# 3-Klassen-Multiclass (siehe generate_fixture.R) -> NULL, siehe Original
# (health_condition-Template) fuer die volle Begruendung.
positive_class <- NULL

# Experiment-Tracking (SQLite, siehe db_schema.sql/db_logging.R)
project_name <- "ci-smoke-test"

seed <- 42
# CI-Fixture hat nur 800 Zeilen (siehe generate_fixture.R) - volle Menge
# verwenden, kein Subset noetig (analog zu steel-plates-fault).
subset_fraction <- 1.0
...
ranger_tuning_search_trees <- 20  # CI-Smoke-Test: nur der Code-Pfad zaehlt, nicht gute Hyperparameter
ranger_tuning_evals <- 4
ranger_tuning_final_trees <- 20
...
lightgbm_tuning_search_iterations <- 20  # CI-Smoke-Test: nur der Code-Pfad zaehlt
lightgbm_tuning_evals <- 4
lightgbm_tuning_final_iterations <- 20
...
generalization_gap_n_boot <- 30  # CI-Smoke-Test: nur der Code-Pfad zaehlt
...
split_sensitivity_repeats <- 5L  # CI-Smoke-Test: nur der Code-Pfad zaehlt
...
learning_curve_repeats <- 2L  # CI-Smoke-Test: nur der Code-Pfad zaehlt
...
seed_stability_n_seeds <- 3L  # CI-Smoke-Test: nur der Code-Pfad zaehlt
seed_stability_n_jitter <- 3L
seed_stability_cv_folds <- 3L
