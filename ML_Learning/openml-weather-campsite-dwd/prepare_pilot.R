# Prepare leakage-controlled OpenML/DWD classification pilots.
#
# Runs the same baseline-vs-weather feature preparation for two independent
# (Bundesland, DWD-station) pairs, so the enrichment technique's effect can
# be checked for robustness across a different region/climate instead of
# relying on a single state. Brandenburg/Potsdam keeps its original,
# unsuffixed output filenames (pilot_baseline.csv etc.) for backward
# compatibility with the existing evidence writeup; Bayern/Muenchen writes to
# *_bayern.csv siblings.
...
prepare_pilot_case <- function(bundesland, dwd_path, dwd_station_id,
                                dwd_station_name, output_suffix) {
  if (!file.exists(source_path) || !file.exists(dwd_path)) {
    stop("OpenML- oder DWD-Quelldatei fehlt fuer ", bundesland, ".")
  }
  ...
  openml <- openml[land == bundesland]
  openml[, date := as.IDate(date)]
  setorder(openml, date)

  # The label is next month's overnight demand. The threshold is frozen on
  # the chronological training portion, not estimated from the test period.
  ...
  suffixed <- function(name) {
    file.path(project_dir, paste0(name, output_suffix, ".csv"))
  }

  fwrite(base, suffixed("pilot_baseline"))
  fwrite(weather, suffixed("pilot_weather"))
  fwrite(data.table(
    openml_dataset_id = 46263L,
    openml_dataset = "weather_and_campsite_germany",
    bundesland = bundesland,
    dwd_station_id = dwd_station_id,
    dwd_station = dwd_station_name,
  ...
  ), suffixed("pilot_manifest"))

  cat("Pilot-Daten erzeugt (", bundesland, "/", dwd_station_name, "):\n", sep = "")
...
prepare_pilot_case(
  bundesland = "Bayern",
  dwd_path = file.path(
    project_dir, "dwd_muenchen",
    "produkt_klima_tag_19540601_20251231_03379.txt"
  ),
  dwd_station_id = 3379L,
  dwd_station_name = "Muenchen-Stadt",
  output_suffix = "_bayern"
)
